...
import { Compose } from "~renderer/common/context";
 ...
 */
const dataSource = ({ name, value, options, onChange }) => {
  const codeRef = useRef();
  const [form] = Form.useForm();
  const { datavApiEnum, datavSqlEnum } = store.getState().component;
  const [state, dispath] = useStore({
    ...
    code: { data: null },
    apiValue: null,
    apiList: datavApiEnum,
    sqlValue: null,
    sqlList: datavSqlEnum
  });
